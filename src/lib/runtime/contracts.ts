...
export const RuntimeActionSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("range_update"), low: z.number().int().nonnegative(), high: z.number().int().min(-1) }).strict(),
...
export const RuntimeStateSchema = z.discriminatedUnion("kind", [
  ...
  z.object({ kind: z.literal("range-explorer"), low: z.number().int().nonnegative(), high: z.number().int().min(-1), mid: z.number().int().nonnegative().nullable(), step: z.number().int().nonnegative(), status: z.enum(["active", "found", "absent"]) }).strict(),
