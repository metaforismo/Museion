  ...
  return (
    <div className="mx-auto min-h-svh w-full max-w-5xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div><div className="flex flex-wrap items-center gap-2"><span className="rounded-md bg-correct-soft px-3 py-1 text-xs font-semibold uppercase tracking-wide text-correct">{session.mode === "verified-replay" ? "Sample lesson" : "From your source"}</span><span className="rounded-md bg-lapis-soft px-3 py-1 text-xs font-medium text-lapis-dark">No account needed · answers checked by code</span></div><h1 className="mt-4 font-display text-3xl font-semibold tracking-[-0.01em] sm:text-4xl">{session.artifact.title}</h1><p className="mt-3 max-w-[62ch] text-lg leading-7 text-ink-soft">{session.artifact.bigQuestion}</p></div>
      ...
      {currentBlock && canAdvance && <button type="button" disabled={busy} onClick={advance} className="sticky bottom-3 mt-5 w-full rounded-xl bg-lapis px-6 py-3 font-semibold text-white shadow-[var(--shadow-2)] transition hover:bg-lapis-dark disabled:opacity-45 lg:static lg:shadow-none">{blockIndex === lessonBlocks.length - 1 ? "Finish lesson and unlock transfer" : "Continue"}</button>}
