...
export default function SiteShell({ children, courses, lessons }: { children: ReactNode; courses: SearchableCourse[]; lessons: SearchableLesson[] }) {
  ...
  return <div className={`min-h-[100dvh] bg-paper lg:grid ${collapsed ? "lg:grid-cols-[5rem_minmax(0,1fr)]" : "lg:grid-cols-[15.5rem_minmax(0,1fr)]"}`}>
    <aside aria-label="Application navigation" className={`fixed inset-y-0 left-0 z-30 hidden border-r border-ink/10 transition-[width] lg:block ${collapsed ? "w-20" : "w-[15.5rem]"}`}><AppSidebar collapsed={collapsed} courses={courses} onToggle={toggle} /></aside>
    ...
    {sidebarOpen && <div className="fixed inset-0 z-40 lg:hidden"><button type="button" aria-label="Close navigation" onClick={() => setSidebarOpen(false)} className="absolute inset-0 bg-ink/30"/><aside ref={mobileDrawer} id="mobile-app-navigation" aria-label="Application navigation" className="relative h-full w-[min(19rem,86vw)] border-r border-ink/10 bg-surface shadow-2xl"><button ref={closeButton} type="button" onClick={() => setSidebarOpen(false)} className="absolute right-3 top-3 z-10 min-h-11 rounded-lg px-3 text-sm font-medium text-ink-soft">Close</button><AppSidebar courses={courses} onNavigate={() => setSidebarOpen(false)} /></aside></div>}
