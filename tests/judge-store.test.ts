...
async function completeLesson(sessionId: string, ownerId: string) {
  await dispatchJudgeAction({ sessionId, ownerId, blockId: "prediction_discard", action: { kind: "prediction_select", optionIndex: 0 } });
  await dispatchJudgeAction({ sessionId, ownerId, blockId: "range_boundary", action: { kind: "range_update", low: 4, high: 6 } });
  await dispatchJudgeAction({ sessionId, ownerId, blockId: "range_boundary", action: { kind: "range_confirm_found", index: 5 } });
  await dispatchJudgeAction({ sessionId, ownerId, blockId: "trace_absent", action: { kind: "trace_submit", low: 0, high: 1, mid: 0 } });
  await dispatchJudgeAction({ sessionId, ownerId, blockId: "trace_absent", action: { kind: "trace_submit", low: 1, high: 1, mid: 1 } });
  await dispatchJudgeAction({ sessionId, ownerId, blockId: "sequence_reasoning", action: { kind: "sequence_submit", order: ["compare_mid", "prove_region", "move_past_mid", "check_shrink"] } });
}
...
describe("verified judge session store", () => {
  ...
  it("keeps transfer locked until every interactive block is complete", async () => {
    ...
    await completeLesson(session.sessionId, "learner-a");
